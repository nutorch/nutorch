# Nutorch v1 (archived) — Architecture Record

> **This documents the archived v1 proof of concept**, a Nushell plugin frozen
> as the reference implementation for v2 (nutorchd). Do not develop in `v1/`; v2
> work ports from it. The active agent contract is the root
> [AGENTS.md](../AGENTS.md). Paths in this document are relative to `v1/` unless
> prefixed with `v1/`.

## Project Overview

**Nutorch v1 is a proof-of-concept Nushell plugin** that demonstrates how to
bring PyTorch tensor operations to the command line by wrapping tch-rs (Rust
bindings for LibTorch, PyTorch's C++ backend).
...
**See [TODO.md](TODO.md) for v1's implementation status and quality metrics.**
...
- **Quality criteria**: Test coverage, helper usage, validation, etc. for each
  method
- **Progress to 1.0**: What needed to be done for each method to be
  production-ready
...
```
v1/
├── README.md                    # v1 user documentation (archived)
├── AGENTS.md                    # This file (v1 architecture record)
├── TODO.md                      # v1 quality/completeness tracker
│
├── cargo/                       # ⭐ The Rust plugin implementation
...
│       └── *.nu
│
└── raw-images/                  # Screenshots of the v1 plugin UX
    └── *.png
```
...
- `cargo/` and `npm/nn.nu/` have their own chat files

The chat files are historical records from before the Issues and Experiments
workflow existed. Do not add to them; do not modify or delete them.
...
cd v1/cargo
cargo build --release
# Binary: v1/cargo/target/release/nu_plugin_torch
```

### Testing

**Important**: Tests must be run from the `v1/cargo/test` directory using the
`test.nu` framework.
...
1. `cd v1/cargo/test` - Navigate to test directory
...
4. **No model serialization** - Can't save/load trained models
